...
module.exports = transformedSvgsDir => icon => {
  ...
  icon.transformedFileName = `${icon.id}-${fileHash.digest('hex').slice(0, 5)}.svg`;
  ...
  return writeFile(path.resolve(transformedSvgsDir, icon.transformedFileName), icon.svg).then(() => icon);
};
